export const generateERDiagram = (tables: any[], relationships: any[]): string => {
  ...
  tables.forEach(table => {
    ...
    table.columns.forEach((column: any) => {
      ...
      if (column.is_primary_key) columnDef += ' PK';
      if (column.is_foreign_key) columnDef += ' FK';
      if (!column.is_nullable) columnDef += ' "NOT NULL"';
      diagram += `${columnDef}\n`;
    });
    diagram += '    }\n';
  });
  ...
  relationships.forEach(rel => {
    const relationshipSymbol = rel.relationship_type === 'one-to-many' ? '||--o{' : 
                              rel.relationship_type === 'many-to-one' ? '}o--||' :
